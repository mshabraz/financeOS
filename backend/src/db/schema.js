...
const MIGRATION_V35 = {
  ...
  up: (db) => {
    ...
    } = require('../services/crossLedgerDedup');

    const revolutRows = db.prepare(
      `SELECT date, amount, description FROM revolut_transactions WHERE amount < -0.01`,
    ).all();
    const crossLedger = removeBankRowsMatchingRevolutCharges(db, revolutRows);
    const zeroBank = removeZeroAmountBankRows(db);
    const zeroRevolut = removeZeroAmountRevolutRows(db);
    console.log(
      `[DB] v35 cross-ledger: bank_removed=${crossLedger} zero_bank=${zeroBank} zero_revolut=${zeroRevolut}`,
    );
  },
};
...
// ── Migration v37: Re-dedupe bank txs (ref suffix -1, merchant in content key) ─
const MIGRATION_V37 = {
  version: 37,
  up: (db) => {
    const { dedupeBankTransactions } = require('../services/bankDedup');
    const bank = dedupeBankTransactions(db);
    console.log(`[DB] v37 bank dedupe: removed=${bank.removed} fp_updated=${bank.fingerprintsUpdated}`);
  },
};

// ── Migration v38: Default OB historical sync window ────────────────────────────
const MIGRATION_V38 = {
  version: 38,
  up: (db) => {
    db.prepare(
      `INSERT OR IGNORE INTO app_settings (key, value) VALUES ('open_banking_sync_backfill_days', '365')`,
    ).run();
  },
};

// ── Migration v39: Auto-sync connected banks on login ─────────────────────────
const MIGRATION_V39 = {
  version: 39,
  up: (db) => {
    db.prepare(
      `INSERT OR IGNORE INTO app_settings (key, value) VALUES ('open_banking_auto_sync_on_login', 'true')`,
    ).run();
  },
};

// ── Migration v40: PayPal manual balance for net worth ──────────────────────────
const MIGRATION_V40 = {
  version: 40,
  up: (db) => {
    db.prepare(
      `INSERT OR IGNORE INTO manual_balances (key, label, icon, amount) VALUES ('paypal', 'PayPal', '💳', 0)`,
    ).run();
  },
};
